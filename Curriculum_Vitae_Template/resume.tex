...
\section{}
  \resumeSubHeadingListStart
    ...
    파이프라인 구축에 탄탄한 배경을 갖추고 있습니다. 코드 레벨로 딥다이브하여 근본 원인을 파악하는 것을 \\ 선호합니다. 특히 트러블슈팅에 강점을 가지고 있어
    안정적이고 확장 가능한 데이터 인프라 구축을 통해  \\ 비즈니스 가치 창출에 기여합니다.
    }
  ...
  \resumeSubHeadingListStart
    \resumeProject
      ...
      \resumeItemListStart
        ...
        \resumeItem{n8n AI Agent 워크플로우 구축으로 자동 분석 결과를 Slack으로 전송}
        \resumeItem{사람의 수동 분석 시간 100\% 감소}
      \resumeItemListEnd
    ...
    \resumeProject
      {액션로그 파이프라인}
         ...
         \resumeItemListStart
            \resumeItem{Kafka Connect 기반 스트리밍 데이터 수집 및 Amazon Glue Crawler로 1시간 주기 준실시간 파티션 등록}
            \resumeItem{메달리온 아키텍처 설계 및 클라이언트 조회 패턴 분석 기반 파티셔닝/버켓팅 최적화}
            \resumeItem{Spark 작업 시간 35\% 단축 (22분→15분)}
         \resumeItemListEnd
    ...
    \resumeProject
      ...
      \resumeItemListStart
        \resumeItem{벡터 유사도와 텍스트 유사도 기반 침해곡 자동 탐지 시스템 구축}
        \resumeItem{Polars와 Kubernetes 조합으로 하루 운영 비용 73\% 절감, 총 소요시간 53\% 단축 (14시간→6.5시간)}
        \resumeItem{Polars와 벡터 데이터베이스(PgVector)를 적용하여 동/변조곡 탐지 정확도 18\%p(5\% $\rightarrow$ 23\%) 향상}
    \resumeItemListEnd
    ...
    \resumeItemListStart
        \resumeItem{Observability 강화를 위한 Prometheus와 Grafana 기반 모니터링 시스템 구축}
        \resumeItem{Presto에서 Trino로의 전환 후 쿼리 처리 속도 15\% 증가, 전환 과정에서 발생한 다양한 운영 이슈(OOM, 느린 쿼리)를 체계적으로 해결하여, 멀티테넌트 환경에서의 안정적인 서비스 제공}
    \resumeItemListEnd
    ...
    \resumeProject
      ...
      \resumeItemListStart
        \resumeItem{워크로드 특성 분석을 통해 분산 처리가 과도함을 확인하고 단일 노드 쿼리 엔진으로 전환}
        \resumeItem{집계 작업 수행 시간 85.42\% 감소 (4분 5초 → 36초), 전체 소요 시간 10분 → 1분}
        \resumeItem{하드웨어 스펙 축소: 2노드(8 코어) → 단일 노드(4 코어), 메모리 48GB → 12GB}
        \resumeItem{월간 운영 비용 71.13\% 절감 (\$19.81 → \$5.72)}
      \resumeItemListEnd
...
\section{기술 스택}
  \resumeSubHeadingListStart
    ...
    \begin{itemize}[leftmargin=1em]
        \item Kubernetes 1.3: Pod, Deployment 리소스 개발
        \item Docker (★★★★☆)
    \end{itemize}
