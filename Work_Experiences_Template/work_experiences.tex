...
\section{\textbf{1. Spark History MCP + AI Agent로 Spark 분석 자동화}}
  \resumeProjectListStart
    ...
    \textbf{상황}: 매일 80 개 정도의 Spark 배치 작업을 운영하는 환경에서 작업 실패 시, 수동으로 Spark UI, CloudWatch Logs, EMR 콘솔을 오가며 수시간씩 디버깅해야 하는 비효율적인 운영 상황이 지속되고 있었습니다.
    
    \textbf{과제}: Spark 작업 실패 시 자동으로 원인을 분석하고 해결책을 제시하는 End-to-End 자동화 시스템을 구축하여, 수동 디버깅 시간을 획기적으로 단축하고 운영 효율성을 향상시키는 것이 목표였습니다.
    
    SparkListener, Spark History MCP, AI Agent를 결합한 자동 분석 시스템을 구축했습니다. Spark 작업 실패 시 자동으로 감지하여 AI가 Spark History Server의 메트릭을 조회하고 원인 분석 및 해결책을 제시하는 End-to-End 파이프라인을 n8n으로 구성하고, 분석 결과를 Slack으로 자동 전송하는 시스템을 구현했습니다.
    ...
    Py4J를 활용하여 Spark 작업 실행 중 발생하는 모든 이벤트(Stage 실패, OOM, Data Skew 등)를 실시간으로 감지하여 n8n Webhook으로 전송하는 SparkListener를 개발했습니다.
    ...
    n8n을 활용하여 Spark 에러 감지부터 분석 결과 전송까지의 전체 워크플로우를 자동화했습니다. HTTP Webhook으로 Spark 에러 정보를 수신하고, OpenAI GPT-5-mini 모델을 사용한 AI Agent가 Spark History MCP를 통해 메트릭을 조회하여 OOM 등 원인을 분석하고 구체적인 해결책(설정 변경값, 단기/장기 전략)을 제시하도록 구성했습니다.
...
\section{\textbf{2. 액션로그 파이프라인}}
  \resumeProjectListStart
    ...
    \textbf{상황}: 대규모 로그 데이터(하루 2.5억건)가 발생하는 환경에서 기존 데이터 처리 시스템의 저장 방식으로는 쿼리 속도가 느렸고, 비즈니스에서 액션로그에 대한 실시간 조회 요구사항이 새롭게 발생한 상황이었습니다.
    
    \textbf{과제}: 느린 쿼리 속도 문제를 해결하고 실시간 조회 요구사항을 만족시킬 수 있는 End-to-End 데이터 파이프라인을 구축하여, 액션로그를 1시간 주기로 준실시간 조회할 수 있는 시스템을 제공하는 것이 목표였습니다.
...
\end{itemize}
\begin{itemize}
    \item Silver 티어를 파티셔닝, 버켓팅으로 저장하여 소규모 파일 문제를 해결하고, Silver 티어의 쿼리 속도를 약 2.5배 향상시켰습니다.
\end{itemize}
    ...
    \resumeSubProjectListStart
        ...
        메달리온 아키텍처를 적용하여 로그 데이터를 여러 단계로 분리 및 정제함으로써, 데이터 품질과 신뢰성을 체계적으로 개선하였습니다. 
        
        액션로그 클라이언트들, 하위 ETL Job들의 조회 패턴을 분석하여, 그 결과를 토대로 파티셔닝과 버켓팅 전략을 설계하고 적용했습니다. 원시 데이터 보존을 통해 언제든 재처리가 가능하고, 각 단계별 데이터 검증(null 처리, 중복 처리, 데이터 타입 변환)을 통해 오류 데이터는 낮은 단계에 격리하여 상위 단계에서는 정확하고 고품질 데이터를 확보하였습니다. 이로써 분석과 비즈니스 활용에 최적화된 신뢰성 높은 데이터 환경을 구축하였습니다.
        ...
        {
        대용량 액션로그 데이터를 수집하기 위해 Kafka 기반의 스트리밍 플랫폼의 Consumer 그룹을 구축하고, Amazon Glue ETL로 준실시간 파티션 등록과 Spark 배치 처리를 병행하는 하이브리드 아키텍처를 설계했습니다.
...
\section{\textbf{3. 침해곡 탐지 파이프라인 개편}}
  \resumeProjectListStart
    ...
    \textbf{상황}: 유통사에서 신규로 입수되는 오디오(하루 7만$\sim$16만 곡)를 FLO 플랫폼 내 500만 곡 보호 음원과 자동 비교해 침해곡을 판별하는 기존 파이프라인이 과도한 복잡성으로 인한 오버엔지니어링 문제와 낮은 탐지율로 인해 운영 효율성이 저하되는 상황이었습니다.
    
    \textbf{과제}: 대규모 음원 데이터 처리 성능을 개선하고 침해곡 탐지 정확도를 향상시키면서도, 시스템 복잡도를 줄이고 운영 비용을 절감할 수 있는 효율적인 파이프라인을 재구축하는 것이 목표였습니다.
    
    파이프라인은 주기적으로 대규모 신규 데이터를 사전 처리하고, 임베딩 추출과 벡터 유사도 연산 단계를 시스템입니다. 과거 파이썬 기반 배치 시스템의 한계를 극복하고자, 데이터프레임 처리 엔진으로 Polars를 도입해 I/O 병목과 실행 비용을 크게 개선했습니다. 또한 벡터 데이터베이스 적용을 통해 수백만 곡 임베딩 간 유사도 필터링(코사인 거리 등) 처리 속도를 대폭 향상시켰으며, 전체 탐지 파이프라인의 정확도와 운영 효율이 올라갔습니다.
    ...
    \begin{itemize}
        \item 대규모 500만 곡 데이터 환경에서 Polars와 벡터 데이터베이스를 적용하여 동/변조곡 탐지 정확도 18\%p(5\% $\rightarrow$ 23\%) 향상
        \item Polars 기반 고성능 배치 처리와 Kubernetes 리소스 최적화로, 하루 운영 비용 약 73\%(\$12.4 $\rightarrow$ \$3.3, EC2 기준) 절감
        \item 벡터 연산 및 데이터 처리 효율 극대화로, 총 소요 시간 14시간 $\rightarrow$ 평균 6.5시간으로 53\% 단축
    \end{itemize}
%     \begin{table}[ht]
% \centering
...
% \end{table}
    ...
    \resumeSubProjectListStart
        ...
        신규로 유입되는 수십만 곡의 오디오와 500만 곡 규모의 보호음원 간 텍스트 유사도(레벤슈타인)와 임베딩 기반 유사도(코사인 거리) 비교를 벡터 데이터베이스(PgVector)를 통해 병렬 연산 처리량을 극대화했습니다.
        
        \vspace{1mm}
        \textbf{사용한 기술}: Python, PostgreSQL(PgVector), asyncio
        \vspace{2mm}
        }
        \resumeSubProject {\textbf{Polars 기반 파이프라인 리뉴얼 및 운영 효율화}}
        {
        
        레거시 파이썬 기반 데이터프레임 처리 시스템을 Polars 엔진으로 전환하여, 대용량 Parquet 데이터 처리 시간과 I/O 병목을 획기적으로 줄이고, 전체 파이프라인의 실행 비용과 소요 시간을 크게 단축시켰습니다.
...
\section{\textbf{4. \href{https://trino.io/}{Trino} 구축 및 운영}}
  \resumeProjectListStart
    ...
    \textbf{기여도}: Prometheus, Grafana 제외하고 약 70\%
    
    {\textbf{주요 내용}}:
    
    \textbf{상황}: 대규모 전사 멀티테넌트 데이터 플랫폼의 쿼리 엔진으로 사용되던 Amazon EMR 기반 Presto의 운영권이 기존 조직에서 저희 조직으로 이관되었으나, Observability가 전혀 구축되어 있지 않아 성능 모니터링과 장애 대응이 어려운 상황이었습니다.
    
    \textbf{과제}: 부족한 Observability를 구축하여 시스템 가시성을 확보하고, 더 나은 성능과 안정성을 제공하는 Trino로 전환하여 쿼리 처리 성능을 개선하는 것이 목표였습니다.
    
    Trino로 전환하는 프로젝트를 주도하면서 마주쳤던 다양한 문제를 해결하고, 쿼리 속도를 개선하였습니다.
...
\end{figure}
    ...
    \textbf{주요 성과}:
        \begin{itemize}
            ...
            \item Observability 확보를 통해 쿼리 사용량 패턴을 분석하고, 사용량이 적은 시간대에 Worker 노드를 자동 스케일 다운하는 시간 기반 스케일링을 도입하여 불필요한 Worker EC2 비용 월 45\% 절감
        \end{itemize}
...
\end{figure}
    ...
    \resumeSubProject{\textbf{데이터 적재 자동화 및 모니터링 시스템 구축, \textbf{기여도}: 60\%}}
    {
...
\section{\textbf{6. PySpark에서 Polars로의 마이그레이션}}
  \resumeProjectListStart
    ...
    \textbf{상황}: 소규모 EMR 클러스터(2노드)로 집계 워크플로우를 운영하면서, 클러스터 생성에 6분, 실제 집계 작업에 4분이 소요되어 인프라 준비 시간이 실제 작업 시간보다 더 오래 걸리는 비효율적인 상황이었습니다.
    
    \textbf{과제}: 워크로드 특성 분석을 통해 분산 처리의 필요성을 재검토하고, 데이터 기반 의사결정을 통해 PySpark에서 Polars로 전환할 수 있는 근거를 확보하여 성능과 비용을 동시에 최적화하는 것이 목표였습니다. \href{https://superhuman54.github.io/posts/journey-to-polars/}{상세한 분석과 검증 과정}을 통해 단일 노드 쿼리 엔진 채택을 결정했습니다.
    
    워크로드 특성 분석을 통해 분산 처리가 과도하다고 판단하고, 단일 머신 쿼리 엔진인 Polars를 도입하였습니다.
    ...
    \textbf{주요 성과}:
        \begin{itemize}
            ...
            \item 일일 운영 비용 71.13\% 절감 (\$19.81 → \$5.72)
        \end{itemize}
...
\section{\textbf{7. 추천 데이터 HTTP 서버 구축과 운영}}
  \resumeProjectListStart
    ...
    \textbf{상황}: 기존 Flask 기반 HTTP 서버가 DynamoDB 조회가 많은 I/O 바운드 환경에서 약 500ms의 느린 응답 시간을 보였으며, 객체 지향적이지 않은 코드 구조로 인해 유지보수성과 코드 품질이 저하되는 문제가 있었습니다.
    
    \textbf{과제}: I/O 바운드 작업에 최적화되고 더 빠른 성능을 제공하는 FastAPI로 전환하여 응답 시간을 개선하고, 동시에 객체 지향적 설계와 타입 힌팅을 도입하여 코드 품질과 유지보수성을 향상시키는 것이 목표였습니다.
    
    FastAPI의 비동기 처리를 통해 응답 지연 시간을 평균 120ms로 약 76\% 감소시켜 처리 속도와 동시 처리 성능을 대폭 개선했습니다. 또한, 타입 힌팅과 \href{https://python-dependency-injector.ets-labs.org/}{Dependency Injector} 도입으로 코드 품질과 유지보수성도 향상시켰습니다.
