...
\section{\textbf{1. Spark History MCP + AI Agent로 Spark 분석 자동화하기}}
  \resumeProjectListStart
    \resumeProject
    {\textbf{소속}: 드림어스컴퍼니} {기여도: 100\%}
    
    {\textbf{주요 내용}:
    
    \textbf{상황}: 매일 80 개 정도의 Spark 배치 작업을 운영하는 환경에서 작업 실패 시 엔지니어가 수동으로 Spark UI, CloudWatch Logs, EMR 콘솔을 오가며 수시간씩 디버깅해야 하는 비효율적인 운영 상황이 지속되고 있었습니다.
...
\section{\textbf{2. 액션로그 파이프라인}}
  \resumeProjectListStart
    \resumeProject
    {\textbf{소속}: 드림어스컴퍼니} {기여도: Broker를 기준으로 Consumer부터, 약 66\%}
    
    {\textbf{주요 내용}}:
    
    \textbf{상황}: 대규모 로그 데이터(하루 2.5억건)가 발생하는 환경에서 기존 데이터 처리 시스템으로는 신속하고 안정적인 수집·처리·분석이 어려웠으며, 정제되지 않은 방대한 raw 데이터를 분석 가능한 데이터 마트로 구성하는데 오랜 시간이 소요되는 상황이었습니다.
    
    \textbf{과제}: 대용량 로그 데이터를 실시간으로 처리하고 분석할 수 있는 확장 가능하고 안정적인 End-to-End 데이터 파이프라인을 구축하여 데이터 처리 효율성과 신뢰성을 확보하고, 액션로그를 1시간 주기로 준실시간 조회할 수 있는 시스템을 제공하는 것이 목표였습니다.
    
    Kafka와 Amazon Glue Crawler 기반의 실시간 데이터 처리, S3 및 Amazon Glue를 이용한 데이터 레이크 구축, Airflow를 활용한 데이터 적재 작업 자동화와 운영 모니터링, 그리고 효율적인 파티션 설계를 통한 대용량 ETL 작업의 성능 최적화까지 다양한 데이터 엔지니어링 과제 전반을 맡아 수행했습니다.
...
\end{itemize}
\begin{itemize}
    \item S3 파티션 구조와 적재 정책을 최적화하여, 소규모 파일 문제를 해결하고 파티션 단위 처리 효율을 높여 Bronze에서 Silver로 ETL 처리하는 구간의 전체 Spark 작업 시간을 평균 약 35\% (22분 → 15분) 감축하였습니다.
\end{itemize}
    ...
    \resumeSubProjectListStart
        ...
        액션로그 클라이언트들의 조회 패턴을 분석하여 그 결과를 토대로 파티셔닝과 버켓팅 전략을 설계하고 적용했습니다. 원시 데이터 보존을 통해 언제든 재처리가 가능하고, 각 단계별 데이터 검증(null 처리, 중복 처리, 데이터 타입 변환)을 통해 오류 데이터는 낮은 단계에 격리하여 상위 단계에서는 정확하고 고품질 데이터를 확보하였습니다. 이로써 분석과 비즈니스 활용에 최적화된 신뢰성 높은 데이터 환경을 구축하였습니다.
        ...
        \vspace{1mm}
        \textbf{사용한 기술}: Apache Spark, Amazon Glue Crawler, Kafka Connect
        \vspace{2mm}
    }
...
\section{\textbf{3. 침해곡 탐지 파이프라인 개편}}
  \resumeProjectListStart
    \resumeProject
    {\textbf{소속}: 드림어스컴퍼니} {기여도: 임베딩 추출(Tensorflow) 제외하고 모두, 약 80\%}
...
% \end{table}
    ...
    \resumeSubProjectListStart
        ...
        레거시 파이썬 기반 데이터프레임 처리 시스템을 Polars 엔진으로 전환하여, 대용량 Parquet 데이터 처리 시간과 I/O 병목을 획기적으로 줄이고, 전체 파이프라인의 실행 비용과 리포트 출력 시간을 크게 단축시켰습니다.
...
\section{\textbf{4. \href{https://trino.io/}{Trino} 구축 및 운영}}
  \resumeProjectListStart
    \resumeProject
    {\textbf{소속}: 드림어스컴퍼니} {기여도: Prometheus, Grafana 제외하고 약 70\%}
...
\section{\textbf{6. PySpark에서 Polars로의 마이그레이션}}
  \resumeProjectListStart
    ...
    \textbf{상황}: 음악 스트리밍 플랫폼의 "연령별 차트" 집계 워크플로우를 소규모 EMR 클러스터(2노드)로 운영하던 중, 클러스터 생성에 6분, 실제 집계에 4분으로 총 10분이 소요되어 클러스터 생성 시간이 실제 작업 시간보다 긴 비효율적인 상황이었습니다.
    ...
    워크로드 특성 분석을 통해 분산 처리가 과도하다고 판단하고, Apache Arrow 기반의 효율적인 단일 머신 쿼리 엔진인 Polars를 도입하여 약 1분으로 단축했습니다.
