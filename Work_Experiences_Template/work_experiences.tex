...
\section{\textbf{1. Spark History MCP + AI Agent로 Spark 분석 자동화}}
  \resumeProjectListStart
    ...
    \textbf{문제 해결(링크)}:
        \begin{itemize}
            \item \href{https://www.blog-dreamus.com/post/spark-history-mcp-ai-agent%EB%A1%9C-spark-%EB%B6%84%EC%84%9D-%EC%9E%90%EB%8F%99%ED%99%94%ED%95%98%EA%B8%B0}{Spark History MCP + AI Agent로 Spark 분석 자동화}
        \end{itemize}
...
\section{\textbf{3. 침해곡 탐지 파이프라인 개편}}
  \resumeProjectListStart
    ...
    \begin{itemize}
        \item 대규모 500만 곡 데이터 환경에서 텍스트 유사도 연산과 임베딩 연산의 조합으로 동/변조곡 탐지 정확도 18\%p(5\% $\rightarrow$ 23\%) 향상
        \item Polars 기반 고성능 배치 처리와 Kubernetes 리소스 최적화로, 하루 운영 비용 약 73\%(\$12.4 $\rightarrow$ \$3.3, EC2 기준) 절감
        \item 벡터 연산 및 데이터 처리 효율 극대화로, 총 소요 시간 53\%(최대 14시간 $\rightarrow$ 6.5시간) 단축
    \end{itemize}
%     \begin{table}[ht]
% \centering
...
% \end{table}
    ...
    \resumeSubProjectListStart
        ...
        도메인 특화 지식을 보유한 유관 부서와의 긴밀한 협업을 통해, 탐지 정확도를 높이는 핵심 솔루션으로 텍스트 유사도(레벤슈타인) 방식을 채택했습니다. 
...
\section{\textbf{4. \href{https://trino.io/}{Trino} 구축 및 운영}}
  \resumeProjectListStart
    ...
    대규모 전사 멀티테넌트 데이터 플랫폼의 쿼리 엔진으로 사용되던 Amazon EMR 기반 Trino를 운영했습니다. 이 엔진은 고객의 이용권 정산 및 조회같은 중요한 배치 작업에 사용되었습니다.
...
\end{figure}
    ...
    \textbf{주요 성과}:
        \begin{itemize}
            ...
            \item Observability 확보를 통해 쿼리 사용량 패턴을 분석하고, 사용량이 적은 시간대에 Worker 노드를 스케일 다운하는 \textbf{시간 기반 스케일링}을 도입하여 불필요한 Worker EC2 비용 월 45\% 절감
        \end{itemize}
...
\section{\textbf{5. Amazon S3 기반 데이터레이크, 데이터웨어하우스 운영}}
  \resumeProjectListStart
    ...
    \textbf{상황}:
    \begin{itemize}
        \item 시간 기반으로 스케줄링된 파이프라인의 연쇄 작업 관리가 어려웠습니다.
...
\section{\textbf{6. PySpark에서 Polars로의 마이그레이션}}
  \resumeProjectListStart
    ...
    \textbf{과제}:
    \begin{enumerate}
        \item 워크로드 특성 분석을 통해 분산 처리의 필요성을 재검토하고, 데이터 기반 의사결정을 통해 PySpark에서 Polars로 전환할 수 있는 근거를 확보하여 성능과 비용을 동시에 최적화하는 것이 목표였습니다.
    \end{enumerate}
...
\section{\textbf{7. 추천 데이터 HTTP 서버 구축과 운영}}
  \resumeProjectListStart
    ...
    오프라인 추천 시스템 (정적 추천 시스템)을 운영하기 위한 사용자 추천 데이터를 조회하는 HTTP 서버를 구축하였습니다.
    
    \textbf{상황}: 
    \begin{itemize}
        \item API 문서를 수동으로 작성하고 있어 생산성이 저하되는 문제가 있었습니다.
        \item 클라이언트는 MSA 아키텍처로 구성되어 있고, 각 서비스의 처리 시간이 곧 전체 응답 시간인데, 이 서버가 느리면 전체 응답 시간이 느려지는 문제가 있었습니다.
        \item I/O 바운드 작업이 많았고, 모든 I/O 작업이 동기적으로 처리되어 있었습니다.
    \end{itemize}
    
    \textbf{과제}: 
    \begin{enumerate}
        \item 문서를 가급적 자동화하여 생산성을 높이고, 클라이언트와의 협업을 용이하게 하는 것이 목표였습니다.
        \item I/O 바운드 작업을 병렬 처리하여 응답 시간을 개선하는 것이 목표였습니다.
        \item 사내 API의 공통적인 응답 템플릿이 있었지만, API 개발때마다 작성해야하는 번거로움을 해결하는 것이 목표였습니다.
    \end{enumerate}
    ...
    \begin{itemize}
        ...
        \item Swagger UI를 통해 자동으로 API 문서를 생성하여 클라이언트와의 협업을 용이하게 하였습니다.
    \end{itemize}
    }
